  ...
  QWizardPage (parent)
{
  ...
  QLabel *label = new QLabel (tr ("The pfmLoad program is used to create or append to PFM structures and/or create parameter "
                                  "files for use with pfmLoader.  When this program is used t load the PFM structures it calls "
                                  "the pfmLoader program as an external process.  Context sensitive help is available by clicking "
                                  "on the Help button and then clicking, with the Question Arrow cursor, on the field of interest.  "
                                  "You can preload a previously saved parameters file (*.prm) by placing it on the command line.  "
                                  "Press <b>Next</b> to go to the first PFM definition page.<br><br>"
  ...
  prmBox->setWhatsThis (tr ("This box will show the name of the parameter file that was placed on the command line.  " 
                            "If no command was placed on the command line it will say <b>No command line parameter file</b>"));
  ...
  if (!parameter_file.isEmpty ())
    {
      prmlbl->setText (tr ("Command line parameter file: <b>%1</b>").arg (parameter_file));
    }
  else
    {
  ...
  mem->setRange (200000000.0, 2147000000.0);
  mem->setSingleStep (100000000.0);
  mem->setDecimals (0.0);
  mem->setValue ((double) pfm_global->cache_mem);
  ...
  mem->setWhatsThis (tr ("Set the amount of cache memory used for each PFM file to be created or appended to.  "
                         "The value is in bytes and ranges from a minimum of 200,000,000 up to 2,147,000,000.  The default is "
                         "400,000,000.  If you set this to a large value (like 2,147,000,000) and you have a limited amount of "
                         "memory and a large number of PFMs to be built you will end up using swap space and your "
                         "build will be extremely slow ;-)"));
...
int32_t startPage::nextId () const
{
  return (1);
}
