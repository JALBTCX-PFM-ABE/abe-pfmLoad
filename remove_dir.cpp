...
void remove_dir (QString dir)
{
  ...
  if (files.cd (dir))
    {
      ...
      for (int32_t i = 0 ; i < flist.size () ; i++) remove (flist.at (i).absoluteFilePath ().toLatin1 ());
    }
  ...
  for (int32_t j = dirList.size () - 1 ; j >= 0 ; j--)
    {
      files.setFilter (QDir::Files | QDir::Hidden);
      ...
      if (files.cd (dirList.at (j)))
        {
          ...
          for (int32_t i = 0 ; i < flist2.size () ; i++) remove (flist2.at (i).absoluteFilePath ().toLatin1 ());
        }
